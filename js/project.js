...
function renderProjects() {
    ...
    if (currentProjects.length === 0) {
        container.innerHTML = `<div class="empty-state"><i class="fas fa-folder-open"></i><h3>프로젝트가 없습니다</h3><p>새 프로젝트를 생성하여 업무를 관리해보세요.</p><button class="btn-primary" style="margin-top: 1rem;" data-click-action="project-create">첫 번째 프로젝트 만들기</button></div>`;
        return;
    }
    ...
    currentProjects.forEach(proj => {
        ...
        const completeBtnHtml = proj.status !== '완료됨' 
            ? `<button class="btn-success" data-click-action="project-complete" data-record-id="${escapeDataValue(proj.id)}" style="margin-right: 0.5rem;" title="프로젝트 완료 처리"><i class="fas fa-check"></i> 완료</button>`
            : ``;

        const editBtnHtml = proj.status !== '완료됨' 
            ? `<button class="btn-edit-item" data-click-action="project-edit" data-record-id="${escapeDataValue(proj.id)}" title="프로젝트 수정"><i class="fas fa-edit"></i></button>`
            : `<button class="btn-edit-item" data-click-action="project-locked" title="수정 불가 (완료됨)" style="opacity: 0.3; cursor: not-allowed;"><i class="fas fa-lock"></i></button>`;

        container.innerHTML += `
            <div class="project-card search-target" data-click-action="project-open" data-record-id="${escapeDataValue(proj.id)}">
                <div class="project-header">
                    <span class="badge ${statusColor}">${escapeHtml(proj.status)}</span>
                    <div style="display: flex; align-items: center; gap: 0.25rem;">
                        ${completeBtnHtml}
                        ${editBtnHtml}
                        <button class="btn-delete-item" data-click-action="project-delete" data-record-id="${escapeDataValue(proj.id)}" title="프로젝트 삭제"><i class="fas fa-trash"></i></button>
                    </div>
                </div>
...
function renderProjectDetail(projId) {
    ...
    const completeBtnHtml = proj.status !== '완료됨'
        ? `<button class="btn-success" data-click-action="project-complete" data-record-id="${escapeDataValue(proj.id)}" title="프로젝트 완료 처리" style="padding: 0.5rem 0.9rem; font-size: 0.85rem;"><i class="fas fa-check"></i> 완료</button>`
        : ``;

    const editBtnHtml = proj.status !== '완료됨'
        ? `<button class="btn-secondary" data-click-action="project-edit" data-record-id="${escapeDataValue(proj.id)}" title="프로젝트 수정" style="padding: 0.5rem 0.9rem; font-size: 0.85rem;"><i class="fas fa-edit"></i> 수정</button>`
        : `<button class="btn-secondary" data-click-action="project-locked" title="수정 불가 (완료됨)" style="opacity: 0.4; cursor: not-allowed; padding: 0.5rem 0.9rem; font-size: 0.85rem;"><i class="fas fa-lock"></i> 수정 불가 (완료됨)</button>`;

    const deleteBtnHtml = `<button class="btn-secondary text-danger" data-click-action="project-delete" data-record-id="${escapeDataValue(proj.id)}" title="프로젝트 삭제" style="padding: 0.5rem 0.9rem; font-size: 0.85rem;"><i class="fas fa-trash"></i> 삭제</button>`;
    ...
    if (tasks.length === 0) {
        tasksHtml = `
            <div class="empty-state project-detail-empty-state" style="padding: 4rem 1.5rem; background: var(--bg-surface); border: 1px dashed var(--border-color); border-radius: var(--radius-lg); text-align: center;">
                ...
                <button class="btn-primary" data-click-action="task-create" data-project-id="${escapeDataValue(proj.id)}" style="display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.65rem 1.25rem; font-size: 0.9rem; font-weight: 500;">
        ...
        tasks.forEach(task => {
            ...
            tasksHtml += `
                <div class="project-detail-task-card search-target${isOverdue ? ' is-overdue' : ''}" data-click-action="task-open" data-task-id="${escapeDataValue(task.id)}" style="cursor: pointer;">
                    <div class="project-detail-task-main">
                        ...
                        <div style="display: flex; align-items: center; gap: 0.25rem;">
                            <button class="btn-delete-item" data-click-action="task-delete" data-task-id="${escapeDataValue(task.id)}" title="작업 삭제">
                    ...
                    <div class="project-detail-task-meta-row">
                        ...
                        <div class="project-detail-task-status-control" data-click-action="ignore">
                            ...
                            <select class="form-control" style="padding: 0.2rem 1.6rem 0.2rem 0.6rem; height: 30px; font-size: 0.775rem; width: auto; min-width: 95px; border-radius: var(--radius-md);" data-change-action="task-status" data-task-id="${escapeDataValue(task.id)}">
    ...
    const addTaskBtnHtml = tasks.length > 0
        ? `<button class="btn-primary" data-click-action="task-create" data-project-id="${escapeDataValue(proj.id)}">
    ...
    container.innerHTML = `
        <div class="project-detail-nav">
            <button class="btn-secondary btn-back" data-click-action="project-detail-close">
